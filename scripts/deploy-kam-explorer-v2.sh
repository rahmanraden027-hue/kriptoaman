...
def route(path, filename, header, version='1', connect="'self'"):
    return f'''    location = {path} {{\n        limit_except GET {{ deny all; }}\n        root /etc/nginx/templates;\n        try_files /kam-dashboard/{filename} =404;\n        default_type text/html;\n{headers}        add_header {header} "{version}" always;\n        add_header Content-Security-Policy "default-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; connect-src {connect}; img-src 'self' data: https://kriptoaman.com; object-src 'none'; base-uri 'self'; form-action 'self'; frame-src 'none'; frame-ancestors 'self'; upgrade-insecure-requests" always;\n    }}\n'''
