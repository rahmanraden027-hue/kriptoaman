...
test('ZEVARYQ UI uses verified live data and never invents market values', async () => {
  const [card, endpoint, metadata] = await Promise.all([
    read('src/components/wallet/KAMTokenCard.jsx'),
    read('functions/api/zevaryq/network-status.js'),
    read('public/zevaryq-mainnet.json'),
  ]);
  assert.match(card, /\/api\/zevaryq\/network-status/);
