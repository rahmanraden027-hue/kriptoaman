...
test('ZEVARYQ global roadmap is registered as a public page', async () => {
  ...
  assert.match(app, /ZEVARYQ Global Roadmap/);
  assert.match(app, /Technology · Transparency · Governance · Verifiable Progress/);
  assert.doesNotMatch(app, /fixed bottom-5 right-4/);
});

test('ZEVARYQ roadmap keeps US$29.37 classified as an indicative scenario', async () => {
...
test('ZEVARYQ roadmap is polished for mobile and exposes the official PDF', async () => {
  const page = await read('src/pages/KAMGlobalRoadmap.jsx');
