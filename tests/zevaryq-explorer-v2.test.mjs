...
test('runtime code is syntactically valid and maintains an exact transaction provenance contract',()=>{
 ...
 assert.equal(tx.kind,'','method names alone do not prove a swap');
 assert.equal(tx.method,'swap');
 ...
 assert.equal(normalize({...sample,timestamp:'invalid'}),null);
 assert.equal(normalize({...sample,timestamp:null}),null);
 assert.equal(normalize({...sample,timestamp:'2999-01-01T00:00:00Z'}),null);
 assert.match(js,/Method names are unverified/);
