...
const checks = {
  ...
  rpcSentrySeparate: { ok: evidence.rpcSentry?.separateFromValidators === true && Boolean(evidence.rpcSentry?.hostFingerprint) },
  evidenceFresh: {
    ok: Number.isFinite(Date.parse(evidence.checkedAt)) && Date.now() - Date.parse(evidence.checkedAt) <= 24 * 60 * 60 * 1000,
