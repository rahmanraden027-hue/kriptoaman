...
contract KAMRouter {
    ...
    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, "KAMRouter: EXPIRED");
        _;
    }
    ...
    function _optimalAmounts(
        ...
        require(amountADesired >= amountAMin, "KAMRouter: A_DESIRED_LT_MIN");
        require(amountBDesired >= amountBMin, "KAMRouter: B_DESIRED_LT_MIN");
    ...
    function _addLiquidity(
        ...
        require(to != address(0), "KAMRouter: ZERO_TO");
    ...
    function addLiquidityKAM(
        ...
        address to,
        uint256 deadline
    ) external payable ensure(deadline) returns (uint256 amountToken, uint256 amountKAM, uint256 liquidity) {
        require(to != address(0), "KAMRouter: ZERO_TO");
        ...
        (amountToken, amountKAM) =
            _optimalAmounts(amountTokenDesired, msg.value, amountTokenMin, amountKAMMin, reserveToken, reserveWKAM);
    ...
    function removeLiquidityKAM(
        ...
        address to,
        uint256 deadline
    ) external ensure(deadline) returns (uint256 amountToken, uint256 amountKAM) {
        require(to != address(0), "KAMRouter: ZERO_TO");
        (amountToken, amountKAM) =
            removeLiquidity(token, WKAM, liquidity, amountTokenMin, amountKAMMin, address(this), deadline);
