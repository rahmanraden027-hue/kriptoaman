...
import { Bell, BellOff, Plus, Trash2, TrendingUp, TrendingDown, CheckCircle2, AlertTriangle, Radio, Radar } from 'lucide-react';
...
export default function Alerts() {
  ...
  const requestPush = async () => {
    if (!('Notification' in window)) {
      setPushStatus(text.unsupported);
      return;
    }
    const perm = await Notification.requestPermission();
    if (perm === 'granted') {
      setPushEnabled(true);
      setPushStatus(text.granted);
      new Notification('KriptoAman', { body: text.notificationBody });
    } else {
      setPushStatus(text.denied);
    }
      ...
      <div className="mx-auto max-w-6xl space-y-5 px-4 pt-5 sm:px-6 lg:px-8">
        <section className="relative overflow-hidden rounded-[28px] border border-sky-400/15 bg-[#071423]/85 p-5 shadow-[0_24px_80px_rgba(0,0,0,.28)] sm:p-6" aria-labelledby="alerts-title">
          ...
          <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="mb-2 flex items-center gap-2 text-sky-300"><Radar className="h-4 w-4" aria-hidden="true" /><span className="text-[10px] font-extrabold tracking-[0.2em]">KRIPTOAMAN ALERT INTELLIGENCE</span></div>
              <h1 id="alerts-title" className="text-2xl font-extrabold sm:text-3xl">{text.title}</h1>
              <p className="mt-1 text-sm text-slate-400">{text.subtitle}</p>
            </div>
            <div className="flex items-center gap-2">
              <span className={`ka-chip inline-flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold ${connected ? 'text-emerald-300' : 'text-slate-300'}`} title={connected ? text.liveDetail : text.alternateDetail}>
                <Radio className="h-3 w-3" aria-hidden="true" />{connected ? text.liveFeed : text.alternateData}
              </span>
              <button type="button" onClick={() => setShowAdd(!showAdd)} className="flex min-h-11 items-center gap-1.5 rounded-xl border border-sky-400/25 bg-sky-500/15 px-4 py-2 text-sm font-semibold text-sky-200 transition hover:bg-sky-500/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/60"><Plus className="h-4 w-4" aria-hidden="true" />{text.add}</button>
            </div>
          </div>
        </section>
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex min-w-0 items-center gap-3">
                <div className={`flex h-11 w-11 items-center justify-center rounded-2xl border ${pushEnabled ? 'border-emerald-500/25 bg-emerald-500/10' : 'border-slate-700/40 bg-slate-900/50'}`}>
                  {pushEnabled ? <Bell className="h-5 w-5 text-emerald-300" aria-hidden="true" /> : <BellOff className="h-5 w-5 text-slate-400" aria-hidden="true" />}
                </div>
                <div className="min-w-0"><p className="text-sm font-semibold">{text.push}</p><p className="text-xs leading-relaxed text-slate-400">{pushEnabled ? text.enabled : text.disabled}</p></div>
              </div>
              {!pushEnabled ? <button type="button" onClick={requestPush} className="min-h-11 rounded-xl border border-violet-500/25 bg-violet-500/15 px-3 text-xs font-semibold text-violet-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400/60">{text.enable}</button> : <CheckCircle2 className="h-5 w-5 text-emerald-400" aria-label={text.enabled} />}
            </div>
          </section>
          <section className="ka-surface p-4 lg:col-span-4">
            <div className="flex items-center gap-2 text-cyan-300"><Radio className="h-4 w-4" aria-hidden="true" /><span className="text-[10px] font-extrabold tracking-[0.18em]">{text.monitor}</span></div>
            <p className="mt-3 text-lg font-extrabold">{connected ? text.connected : text.snapshot}</p>
            <p className="mt-1 text-xs text-slate-500">{text.monitoring}</p>
          </section>
        </div>
        ...
        {showAdd && (
          <div className="ka-surface space-y-3 p-5">
            <p className="font-semibold text-white">{text.newAlert}</p>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="mb-1 block text-xs text-slate-400">{text.coin}</label><select value={form.coin} onChange={e => setForm(f => ({ ...f, coin: e.target.value }))} className="min-h-11 w-full rounded-xl border border-sky-400/15 bg-slate-950/60 px-3 py-2 text-sm text-white">{COINS.map(c => <option key={c}>{c}</option>)}</select></div>
              <div><label className="mb-1 block text-xs text-slate-400">{text.condition}</label><select value={form.condition} onChange={e => setForm(f => ({ ...f, condition: e.target.value }))} className="min-h-11 w-full rounded-xl border border-sky-400/15 bg-slate-950/60 px-3 py-2 text-sm text-white"><option value="above">{text.above}</option><option value="below">{text.below}</option></select></div>
            </div>
            <div><label className="mb-1 block text-xs text-slate-400">{text.target}</label><input type="number" min="0" step="any" value={form.price} onChange={e => setForm(f => ({ ...f, price: e.target.value }))} placeholder={text.example} className="min-h-11 w-full rounded-xl border border-sky-400/15 bg-slate-950/60 px-3 py-2 text-sm text-white" /></div>
            <div className="flex gap-2"><button type="button" onClick={addAlert} className="min-h-11 flex-1 rounded-xl bg-sky-600 py-2.5 text-sm font-semibold text-white hover:bg-sky-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/60">{text.save}</button><button type="button" onClick={() => setShowAdd(false)} className="min-h-11 rounded-xl border border-slate-700 bg-slate-900/60 px-4 py-2.5 text-sm text-slate-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500">{text.cancel}</button></div>
          </div>
        )}

        {alerts.length === 0 && !showAdd && (
          <div className="ka-surface px-6 py-12 text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-3xl border border-sky-500/20 bg-sky-500/10 shadow-[0_0_40px_rgba(56,189,248,.12)]"><Bell className="h-7 w-7 text-sky-300" aria-hidden="true" /></div>
        ...
        <div className="space-y-2">
          {alerts.map(a => (
            <div key={a.id} className={`ka-surface flex items-center justify-between gap-3 p-4 transition ${a.active ? '' : 'opacity-60'}`}>
              <div className="flex min-w-0 items-center gap-3"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${a.condition === 'above' ? 'bg-emerald-500/15' : 'bg-red-500/15'}`}>{a.condition === 'above' ? <TrendingUp className="h-4 w-4 text-emerald-400" aria-hidden="true" /> : <TrendingDown className="h-4 w-4 text-red-400" aria-hidden="true" />}</div><div className="min-w-0"><p className="truncate text-sm font-semibold text-white">{a.coin} {a.condition === 'above' ? '↑' : '↓'} ${Number(a.price).toLocaleString()}</p><p className="truncate text-[11px] text-slate-500">{a.triggeredAt ? text.triggered : a.active ? text.active : text.paused} · {new Date(a.createdAt).toLocaleDateString(language === 'en' ? 'en-US' : 'id-ID')}</p></div></div>
              <div className="flex shrink-0 items-center gap-1"><button type="button" aria-label={a.active ? text.toggleOff : text.toggleOn} onClick={() => toggleAlert(a.id)} className="flex h-11 w-11 items-center justify-center rounded-xl hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/60">{a.active ? <Bell className="h-4 w-4 text-sky-300" aria-hidden="true" /> : <BellOff className="h-4 w-4 text-slate-500" aria-hidden="true" />}</button><button type="button" aria-label={text.remove} onClick={() => deleteAlert(a.id)} className="flex h-11 w-11 items-center justify-center rounded-xl hover:bg-red-500/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400/60"><Trash2 className="h-4 w-4 text-red-400" aria-hidden="true" /></button></div>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-xl border border-yellow-500/20 bg-yellow-500/8 p-3"><p className="text-sm leading-relaxed text-yellow-200"><AlertTriangle className="mr-1 inline h-3.5 w-3.5" aria-hidden="true" />{text.info}</p></div>
      </div>
    </div>
  );
}
