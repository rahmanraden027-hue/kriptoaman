...
export default function Login() {
  ...
  const admin2FARequired = new URLSearchParams(window.location.search).get("admin_2fa") === "1";
  const isAdminEmail = email.trim().toLowerCase() === "kriptoaman@gmail.com";
  const showTwoFactor = admin2FARequired || requires2FA;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (admin2FARequired) {
        await kriptoAuth.loginAdminWith2FA(totpCode);
        window.location.href = "/ServerControl";
        return;
      }
  ...
  return (
    <AuthLayout icon={LogIn} title={text.title} subtitle={text.subtitle} logo darkBlue footer={<>{text.noAccount}{" "}<Link to="/register" className="font-semibold text-sky-400 hover:text-sky-300 hover:underline">{text.register}</Link></>}>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {!admin2FARequired && (
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input id="email" type="email" autoComplete="email" autoFocus={!showTwoFactor} placeholder="you@example.com" value={email} onChange={(e) => { setEmail(e.target.value); setRequires2FA(false); }} className="pl-10 h-12" required disabled={requires2FA} />
            </div>
          </div>
        )}

        {!admin2FARequired && !isAdminEmail && (
          <div className="space-y-2">
        ...
        {admin2FARequired && <div className="rounded-lg border border-sky-500/30 bg-sky-500/10 p-3 text-sm text-sky-200">{text.admin2FAInfo}</div>}
        ...
        {showTwoFactor && (
          <div className="space-y-2 rounded-xl border border-sky-500/30 bg-sky-500/10 p-3">
            ...
            <button type="button" onClick={() => { if (admin2FARequired) { window.location.href = "/login"; return; } setRequires2FA(false); setTotpCode(""); }} className="text-xs text-sky-300 hover:underline">Ganti akun / kata sandi</button>
          </div>
        )}

        {!admin2FARequired && isAdminEmail && <div className="rounded-lg border border-sky-500/30 bg-sky-500/10 p-3 text-sm text-sky-200">{text.adminInfo}</div>}
        {!admin2FARequired && adminLinkSent && <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-200">{text.adminSent}</div>}

        <Button type="submit" className="w-full h-12 font-medium" disabled={loading || (showTwoFactor && totpCode.length !== 6)}>
          {loading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />{text.loading}</> : showTwoFactor ? text.verify : isAdminEmail ? text.adminSubmit : text.submit}
        </Button>
      </form>
    </AuthLayout>
  );
