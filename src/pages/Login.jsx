...
export default function Login() {
  ...
  const isAdminMagic2FA = searchParams.get("admin_2fa") === "1";
  ...
  const verifyAdminMagic2FA = async () => {
    ...
    if (!response.ok || data?.authenticated !== true) {
      throw new Error(data?.error || text.invalid);
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {(requires2FA || isAdminMagic2FA) && (
          <div className="space-y-2 rounded-xl border border-sky-500/30 bg-sky-500/10 p-3">
            ...
            <p className="text-xs text-sky-200/70">{isAdminMagic2FA ? text.adminTwoFactorHelp : text.twoFactorHelp}</p>
            ...
            {!isAdminMagic2FA && <button type="button" onClick={() => { setRequires2FA(false); setTotpCode(""); }} className="text-xs text-sky-300 hover:underline">Ganti akun / kata sandi</button>}
          </div>
        )}

        {!isAdminMagic2FA && isAdminEmail && <div className="rounded-lg border border-sky-500/30 bg-sky-500/10 p-3 text-sm text-sky-200">{text.adminInfo}</div>}
        {!isAdminMagic2FA && adminLinkSent && <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-200">{text.adminSent}</div>}

        <Button type="submit" className="w-full h-12 font-medium" disabled={loading || ((requires2FA || isAdminMagic2FA) && totpCode.length !== 6)}>
          {loading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />{text.loading}</> : isAdminMagic2FA ? text.verify : isAdminEmail ? text.adminSubmit : requires2FA ? text.verify : text.submit}
        </Button>
      </form>
    </AuthLayout>
  );
