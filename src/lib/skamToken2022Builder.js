...
export function metadataPackedLength({ name = SKAM_NAME, symbol = SKAM_SYMBOL, uri = SKAM_METADATA_URI } = {}) {
  // TokenMetadata state: updateAuthority(32) + mint(32) + 3 Borsh strings + Vec additionalMetadata length(4).
  return 32 + 32 + (4 + new TextEncoder().encode(name).length) + (4 + new TextEncoder().encode(symbol).length) + (4 + new TextEncoder().encode(uri).length) + 4;
}

export function mintBaseSpaceWithMetadataPointer() {
  // This mirrors @solana/spl-token getMintLen([ExtensionType.MetadataPointer]):
  // extended accounts use ACCOUNT_SIZE + account-type byte + TLV(type+length+state).
...
export function createInitializeMint2Instruction(mint, authority) {
  // InitializeMint2 uses COption<Pubkey> for freezeAuthority: u32 tag + 32-byte key.
  const data = concatBytes(
