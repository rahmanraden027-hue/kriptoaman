...
export default function GLandingInstitutional() {
  return (
    <section id="institutional" className="px-4 sm:px-6 py-10">
      <div className="max-w-[1440px] mx-auto">
        <div className="max-w-3xl">
          <p className="text-xs font-bold uppercase tracking-[0.18em] ka-blue">Verified Ecosystem</p>
          ...
          {ITEMS.map((item) => {
            ...
            const content = (
              <>
                ...
                <p className="text-xs font-bold uppercase tracking-[0.12em] ka-text2 mt-4">{item.eyebrow}</p>
                <h3 className="font-bold text-base ka-text mt-2">{item.title}</h3>
                <p className="text-sm ka-text2 leading-relaxed mt-2 flex-1">{item.desc}</p>
                <span className="inline-flex items-center gap-2 text-sm font-semibold ka-blue mt-4">
            ...
            return item.href ? (
              <a key={item.title} href={item.href} target="_blank" rel="noopener noreferrer" className="ka-card p-5 flex flex-col min-h-[225px] hover:-translate-y-0.5 transition-transform">
                {content}
              </a>
            ) : (
              <Link key={item.title} to={item.to} className="ka-card p-5 flex flex-col min-h-[225px] hover:-translate-y-0.5 transition-transform">
                {content}
              </Link>
            );
          })}
        ...
        <p className="text-xs ka-text2 mt-5 leading-relaxed">
