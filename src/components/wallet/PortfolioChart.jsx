    ...
    <div className="bg-slate-900 border border-slate-700 rounded-xl px-3 py-2.5 text-xs shadow-xl">
      ...
      <div className="border-t border-slate-700 mt-1.5 pt-1.5">
        ...
        <span className="text-white font-bold">
          {formatUSD(payload.reduce((s, p) => s + (p.value || 0), 0))}
        </span>
      </div>
    </div>
  );
...
export default function PortfolioChart({ addresses = {} }) {
  ...
  const [viewMode, setViewMode] = useState('area'); // 'area' | 'pie'

  useEffect(() => {
    async function load() {
      setLoading(true);

      // 1. Get current prices + balances
      ...
      const vals = {};
      let total = 0;
      COIN_LIST.forEach(coinId => {
        const bal = balMap[coinId];
        const p = prices[coinId]?.price || 0;
        const amount = bal ? parseFloat(formatAmount(coinId, bal.balance || 0)) : 0;
        vals[coinId] = { usd: amount * p, amount, price: p, change24h: prices[coinId]?.change24h };
        total += amount * p;
      });
      setCoinValues(vals);
      setTotalUSD(total);
      ...
      // Weighted 24h change
      if (total > 0) {
        const weightedChg = COIN_LIST.reduce((s, coinId) => {
          const w = vals[coinId].usd / total;
          return s + w * (vals[coinId].change24h || 0);
        }, 0);
        setTotalChange(weightedChg);
      }

      // 2. Fetch historical prices
      const histories = await Promise.all(
        COIN_LIST.map(coinId =>
          fetchCoinHistory(COINS[coinId].coingeckoId, range).catch(() => [])
        )
      );

      // Combine by date
      const dateMap = {};
      COIN_LIST.forEach((coinId, idx) => {
        const hist = histories[idx];
        const amount = vals[coinId].amount;
        hist.forEach(({ date, price }) => {
          if (!dateMap[date]) dateMap[date] = { date };
          dateMap[date][coinId] = parseFloat((amount * price).toFixed(2));
        });
      });

      const combined = Object.values(dateMap)
        .sort((a, b) => a.date < b.date ? -1 : 1)
        .map(d => ({
          ...d,
          total: COIN_LIST.reduce((s, c) => s + (d[c] || 0), 0),
        }));
  ...
  return (
    <div className="bg-slate-800/50 border border-slate-700/30 rounded-2xl p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="text-slate-400 text-xs mb-0.5">Total Portofolio</div>
          {loading ? (
            <div className="h-7 w-36 bg-slate-700/50 rounded animate-pulse" />
          ) : (
            <div className="flex items-center gap-2">
              <span className="text-2xl font-bold text-white">{formatUSD(totalUSD)}</span>
              {totalChange !== null && (
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex gap-0.5 bg-slate-900 border border-slate-700 rounded-lg p-0.5">
            <button
              ...
              className={`p-1.5 rounded-md transition-colors ${viewMode === 'area' ? 'bg-slate-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}
            >
              <BarChart2 className="w-3.5 h-3.5" />
            </button>
            <button
              onClick={() => setViewMode('pie')}
              className={`p-1.5 rounded-md transition-colors ${viewMode === 'pie' ? 'bg-slate-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}
            >
      ...
      {/* Range selector (only for area chart) */}
      {viewMode === 'area' && (
          ...
          {RANGE_OPTIONS.map(({ label, days }) => (
            <button
              ...
              className={`text-xs px-3 py-1 rounded-md transition-colors ${
                range === days ? 'bg-slate-600 text-white' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              {label}
            </button>
            ...
            <AreaChart data={chartData} margin={{ top: 4, right: 0, bottom: 0, left: 0 }}>
              ...
              {COIN_LIST.map(coinId => (
                <Area
                  ...
                  dataKey={coinId}
                  name={COINS[coinId].symbol}
                  stroke={COINS[coinId].color}
                  strokeWidth={1.5}
                  fill={`url(#pg-${coinId})`}
                  dot={false}
                  activeDot={{ r: 3, fill: COINS[coinId].color }}
                  stackId="1"
                />
              ))}
            </AreaChart>
      ...
      <div className="grid grid-cols-3 gap-2 pt-1 border-t border-slate-700/40">
        {COIN_LIST.map(coinId => {
          ...
          const pct = totalUSD > 0 && v ? ((v.usd / totalUSD) * 100).toFixed(1) : '0.0';
          return (
