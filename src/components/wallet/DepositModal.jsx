...
import { X, Copy, CheckCircle2, Loader2, Building2, AlertTriangle, Info, ArrowDownToLine, ShieldCheck, Clock, Upload, Image, RefreshCw } from 'lucide-react';
          ...
          <div className="p-4 space-y-4">
            ...
            {tab === 'crypto' && (
              <div className="space-y-4">
                ...
                <Button onClick={submitCrypto} disabled={!txHash.trim() || !amountCrypto || !selectedCryptoEntry || submitting || (selectedCryptoEntry?.minDeposit > 0 && parseFloat(amountCrypto) < selectedCryptoEntry.minDeposit) || (selectedCryptoEntry?.maxDeposit > 0 && parseFloat(amountCrypto) > selectedCryptoEntry.maxDeposit)}
            ...
            {tab === 'bank' && (
              <div className="space-y-4">
                ...
                <Button onClick={submitBank} disabled={idrRaw < 50000 || !senderName.trim() || !selectedBank || submitting || uploadingProof}
                  ...
                  {submitting || uploadingProof ? <><Loader2 className="w-5 h-5 animate-spin mr-2" />{uploadingProof ? 'Mengupload bukti...' : 'Mengirim...'}</> : <><ArrowDownToLine className="w-4 h-4 mr-2" /> Kirim Bukti Transfer</>}
                </Button>
              </div>
            )}
          </div>
