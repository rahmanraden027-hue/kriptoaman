...
export async function onRequestGet({ request, env }) {
  try {
    ...
    // Preserve passwordless admin sign-in while still requiring the enrolled TOTP.
    // The mailbox link creates only a short-lived, HttpOnly second-factor challenge;
    // no authenticated admin session exists until verify-2fa succeeds.
    if (twoFactorEnabled) {
