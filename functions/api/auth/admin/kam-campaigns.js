...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const id = await createKamCampaign(env.AUTH_DB, admin.id, body || {});
    await recordAdminAudit(env.AUTH_DB, request, admin, 'kam_campaign.create', {
      targetType: 'kam_campaign', targetId: id, metadata: { code: body?.code, type: body?.type, budgetPoints: body?.budgetPoints },
    });
